...
def get_user_data(chat_id):
    if chat_id not in USER_DATA:
        USER_DATA[chat_id] = {
            ...
            'is_monitoring': False # CÔNG TẮC CHỐNG LẶP TIN NHẮN (1 LUỒNG DUY NHẤT)
        }
...
# --- TÍN HIỆU (HARDCORE MODE CHUẨN 3 ĐIỀU KIỆN) ---
                ...
                try:
                    ...
                    if closes is not None:
                        ...
                        if hit_tp or hit_sl:
                            ...
                            # Đã sửa: Lưu text trước, xóa data sau, cuối cùng mới gửi để chặn các bản sao khác
...
def handle_auto(message):
    try:
        ...
        if added:
            ...
            # BẢO VỆ CHỐNG TẠO BẢN SAO LẶP
            if not user.get('is_monitoring', False):
                user['is_monitoring'] = True
                # Thêm daemon=True để luồng tự động chết khi server tắt, dọn sạch rác
...
print("🤖 BOT SIGNAL ĐANG CHẠY (ĐÃ SỬA LỖI LẶP TIN NHẮN)...")
keep_alive()
bot.infinity_polling()            'stats': {'wins': 0, 'losses': 0}
        }
    return USER_DATA[chat_id]
